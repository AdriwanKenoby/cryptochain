...
router.get('/blocks/:id', passport.authenticate('bearer', { session: false }), (req, res) => {
  const { id } = req.params
  const { length } = req.app.locals.blockchain.chain
  const blocksReversed = req.app.locals.blockchain.chain.slice().reverse()
...
router.post('/create-miner', passport.authenticate('bearer', { session: false }), (req, res, next) => {
  const id = req.user._id.toString()
  const wallet = req.app.locals.wallets.get(id)
